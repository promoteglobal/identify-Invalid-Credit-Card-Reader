...
const validateCred = (arr) => {
  ...
  //loop through arrays and determine even from odd numbered credit cards
  for(let i = arr.length - 1; i >= 0; i--) {
    let value = arr[i]; //get values for credit card individual numbers that don't need to be calculated
    ...
    if(sum % 10 === 0) { //return true if valid, false if invalid.
      return true;
    } else {
      return false;
    }
...
//gather an array of all invalid credit card numbers
...
//gather an array of credit card company names that gives out invalid credit card numbers.
const idInvalidCardCompanies = (invalidArrs) => {
  let company = "";
  const invalidCompanies = [];
  for(let i = 0; i < invalidArrs.length; i++) {
    const firstDigit = invalidArrs[i][0];
    switch(firstDigit) {
      case 3:
